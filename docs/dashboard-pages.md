...
看板只展示已采集的稳定状态，不运行测试。页面依次为：总览、Issue、PR、CI、测试、Coverage、版本验证、运维。旧的“看板”“构建报告”“历史数据”页已移除；看板形式并入 Issue 与 PR 页，历史分片仍作为采集器的存储保留。
...
**流水线**：PR 门禁、主干、每日构建、版本构建四张卡片，给出最近一次运行的结论、时间、工作流和近 30 天成功率。下方是近 14 天的分层图，与 CI 页同一数据，[CI 分层历史](ci-history-lanes.md)有详细说明。
...
目录树的数据来自源仓覆盖率摘要中的 `sources` 列表，每项为仓库相对路径加该文件自己的 totals。当前值与整仓结果口径一致：以最新完整基线的文件为底，每个主干增量替换它所测分组下的文件；PR 结果不改变主干视图。摘要没有 `sources` 时，目录树提示暂无逐文件数据，其余视图不受影响。
