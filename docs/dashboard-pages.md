...
**流水线**（默认分支线）：PR 门禁、主干、每日构建、版本构建四张卡片，给出最近一次运行的结论、时间、工作流、耗时和近 30 天成功率。下方是近 14 天的分层图，与 CI 页同一数据，[CI 分层历史](ci-history-lanes.md)有详细说明。
...
版本验证页只把提交 SHA 与版本一致的验证运行关联到版本，并给出 E2E 通过数；没有匹配运行时明确标出，发布成功不代表测试完成。运维页展示分支与保护、安全、社区健康度、提交活跃度和陈旧治理；贡献者直接链接到 GitHub 的贡献者页面（`/graphs/contributors`），看板不再采集贡献者数据。
